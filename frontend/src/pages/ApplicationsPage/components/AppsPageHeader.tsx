...
export function AppsPageHeader({ attentionCount, linkedCount, managedCount }: AppsPageHeaderProps) {
  ...
  return (
    <Surface as="header" className="overflow-hidden border-sky-300/15 bg-[#07142b]/90 shadow-xl shadow-slate-950/20" tone="panel">
      <div className="space-y-1 px-4 pb-3 pt-5 sm:px-5 sm:pt-6">
      ...
      <div className="grid gap-2.5 border-t border-sky-300/10 p-3 sm:grid-cols-3 sm:p-4">
        {metrics.map(({ icon: Icon, key, label }) => (
          <div
            className={cn(
              'flex min-w-0 items-center gap-3 rounded-xl border border-sky-300/15 bg-slate-950/25 px-3 py-2.5',
              key === 'attention' && attentionCount > 0 && 'border-amber-300/30 bg-amber-400/5',
            )}
            key={key}
          >
            <span className={cn(
              'grid size-10 shrink-0 place-items-center rounded-lg border border-cyan-300/15 bg-cyan-400/10 text-cyan-200',
            ...
            <span className="min-w-0">
              <span className="block text-xl font-semibold leading-none text-white">{values[key]}</span>
              <span className="mt-1 block truncate text-xs text-slate-400">{label}</span>
            </span>
          </div>
        ))}
      </div>
