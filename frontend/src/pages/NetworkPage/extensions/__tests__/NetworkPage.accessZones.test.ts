...
test('Access zones place managed apps in readable zones', () => {
  ...
  assert.deepEqual(zones[2].apps.map((app) => app.label), ['Homepage']);
  assert.equal(zones[2].apps[0].external, false);
