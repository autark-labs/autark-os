...
import { useTheme, type ProjectOsThemeId } from '@/contexts/ThemeContext';
import { cn } from '@/lib/utils';
import { projectOsThemes } from '@/lib/themeModel';

const themeSwatches: Record<ProjectOsThemeId, string> = {
...
export function ThemeSelectorPopover({ className }: { className?: string }) {
  ...
  const activeTheme = projectOsThemes.find((option) => option.id === theme) ?? projectOsThemes[0];
          ...
          {projectOsThemes.map((option) => {
            ...
            return (
              <button
                ...
                key={option.id}
                onClick={() => setTheme(option.id as ProjectOsThemeId)}
                type="button"
              >
                <span className={cn('grid size-9 shrink-0 place-items-center rounded-lg bg-gradient-to-br shadow-sm shadow-slate-950/30', themeSwatches[option.id as ProjectOsThemeId])}>
