...
contract_suite="${repo_root}/scripts/tests/release-contract-suite.sh"
[[ -r "${workflow}" ]]
[[ -r "${contract_suite}" ]]
...
grep -q 'release-runner-portability-test.sh' "${contract_suite}"
grep -q 'release-workflow-contract-test.sh' "${contract_suite}"
grep -q 'autark-os-cli-admin-session-test.sh' "${contract_suite}"
grep -q 'autark-os-admin-recovery-test.sh' "${contract_suite}"
grep -q 'autark-os-installer-support-bundle-test.sh' "${contract_suite}"
grep -q 'autark-os-update-helper-test.sh' "${contract_suite}"
grep -q 'service-hardening-contract-test.sh' "${contract_suite}"
grep -q 'service-helper-elevation-contract-test.sh' "${contract_suite}"
grep -q 'service-hardening-drift-test.sh' "${contract_suite}"
...
grep -q 'environment:.*core-release-production.*core-release-staging' "${workflow}"
grep -q 'AUTARK_OS_RELEASE_SIGNATURE_MODE: signed' "${workflow}"
grep -q 'AUTARK_OS_RELEASE_SIGNING_PRIVATE_KEY_B64' "${workflow}"
grep -q 'AUTARK_OS_RELEASE_SIGNING_PUBLIC_KEY_B64' "${workflow}"
grep -q 'AUTARK_OS_RELEASE_TRUST_ROOT_SHA256' "${workflow}"
grep -q 'AUTARK_OS_RELEASE_ORIGIN' "${workflow}"
grep -q 'base64 --decode' "${workflow}"
grep -q 'AUTARK_OS_PRIVATE_SOURCEMAPS: "1"' "${workflow}"
grep -q 'autark-os-private-source-maps-' "${workflow}"
grep -q 'frontend/dist/\*\*/\*.map' "${workflow}"
grep -q 'Release backend jar must not ship frontend source maps' "${workflow}"
