...
output="$(AUTARK_OS_LSBLK_FIXTURE="${lsblk_fixture}" "${repo_root}/scripts/bootstrap-autark-os.sh" \
  ...
  --runtime-dir "/media/jack/Backup Drive/autark-os")"
...
assert storage["runtimePath"] == "/media/jack/Backup Drive/autark-os"
assert storage["runtimeMount"]["mountPoint"] == "/media/jack/Backup Drive"
assert storage["runtimeMount"]["stability"] == "unstable"
assert storage["runtimeMount"]["stableMountSuggestion"] == "/mnt/autark-os-data"
