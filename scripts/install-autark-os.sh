...
RELEASE_URL="${AUTARK_OS_RELEASE_URL:-}"
CHANNEL="${AUTARK_OS_UPDATE_CHANNEL:-stable}"
VERSION="${AUTARK_OS_VERSION:-latest}"
RUNTIME_DIR=""
PORT=""
...
die() {
  printf '[autark-os installer] error: %s\n' "$*" >&2
  exit 1
}
...
run_guided_setup() {
  ...
  if read_yes_no "Use recommended settings" "Y"; then
    ...
    [[ -n "${RUNTIME_DIR}" ]] || RUNTIME_DIR="$(read_default "Runtime data directory" "/var/lib/autark-os")"
...
verify_release() {
  ...
  [[ -x "${bundle_dir}/scripts/bootstrap-autark-os.sh" ]] || die "Release bundle is missing scripts/bootstrap-autark-os.sh"
...
Recovery and rollback notes:
  ...
  - Inspect service status: autark-os status
  - Follow backend logs: autark-os logs
...
run_plan() {
  ...
  "${bundle_dir}/scripts/bootstrap-autark-os.sh" "${args[@]}"
}
...
main() {
  ...
  release_env="${bundle_dir}/autark-os-release.env"
  release_version="$(metadata_value AUTARK_OS_VERSION "${release_env}")"
  release_channel="$(metadata_value AUTARK_OS_UPDATE_CHANNEL "${release_env}")"
