...
validate_build_architecture() {
  ...
  [[ "${ARTIFACT_ARCHITECTURE}" == "${host_architecture}" ]] && return 0
  [[ "${SKIP_BUILD}" -eq 1 && -d "${AUTARK_OS_RUNTIME_DIR:-}" ]] ||
    die "Cannot build ${ARTIFACT_ARCHITECTURE} release artifacts on ${host_architecture}. Cross-architecture packaging requires --skip-build and AUTARK_OS_RUNTIME_DIR containing a verified ${ARTIFACT_ARCHITECTURE} runtime."
}
...
build_runtime() {
  ...
  run_cmd jlink --add-modules java.base,java.compiler,java.desktop,java.instrument,java.logging,java.management,java.naming,java.net.http,java.security.jgss,java.sql,java.transaction.xa,java.xml,jdk.crypto.ec,jdk.management,jdk.unsupported --strip-debug --no-header-files --no-man-pages --output "${runtime_dir}"
}
