...
Environment overrides:
  ...
  AUTARK_OS_BUILD_SHA, AUTARK_OS_BUILD_DATE
USAGE
}
...
write_env_file() {
  ...
  tmp_file="$(mktemp)"
  if [[ -f "${env_file}" ]]; then
    grep -v -E '^(AUTARK_OS_RUNTIME_ROOT|AUTARK_OS_INSTALL_DIR|AUTARK_OS_CONFIG_DIR|AUTARK_OS_LOG_DIR|AUTARK_OS_BACKEND_JAR|AUTARK_OS_FILEOPS_HELPER|AUTARK_OS_VERSION|AUTARK_OS_BUILD_SHA|AUTARK_OS_BUILD_DATE|SERVER_PORT|LOGGING_FILE_NAME|AUTARK_OS_SETUP_COMMAND)=' "${env_file}" >"${tmp_file}" || true
  fi
  cat >>"${tmp_file}" <<EOF
