...
The AMD64 and ARM64 bundles use pinned, checksum-verified Temurin 21 JREs
cached under `build/cache/`; the workstation JDK is used only to compile and
test the application. This avoids accidentally bundling Ubuntu's system
OpenJDK when it requires a newer glibc than Debian 12. The release builder
still verifies every bundled runtime ELF file against the declared host
baseline before packaging.
...
The ARM64 runtime is copied into the bundle and never executed on this
workstation. This avoids QEMU, Docker Buildx, cross-compiling, and Pi-side build
work while preserving the same installer and application payload.
