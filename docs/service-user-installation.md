...
The systemd service uses a private temporary directory, read-only system and kernel-control paths, inaccessible kernel logs/modules, protected home/control-group paths, a closed device policy, a six-capability bound for the existing helper, a restrictive umask, and explicit writable runtime, log, and configuration paths. The bound allows sudo identity/audit handling, traversal of protected runtime data, and restoration of file ownership and modes (`CHOWN` and `FOWNER`). Ambient capabilities remain empty. The configuration exception is only for the root-owned approved-backup destination record.
...
Debian's [systemd execution documentation](https://manpages.debian.org/bookworm/systemd/systemd.exec.5.en.html) explains that several sandbox switches implicitly enable `NoNewPrivileges` for non-root services. The profile therefore uses mount restrictions, `DevicePolicy=closed`, and the capability bound instead of those seccomp-based switches. It does not claim syscall, personality, or socket-family filtering. Qualification must exercise the helper from the actual systemd service; checking the text `NoNewPrivileges=false` or invoking sudo from an administrator shell is insufficient.
