...
## Managed app changes during beta

New managed-app updates are deferred for this controlled beta. Autark-OS does
not silently update managed apps, and it does not offer a new update action or
create an update job.

Existing release snapshots and rollback records remain available as recovery
evidence. If an app already has such a record, review the recovery plan in
**My Apps** or **Diagnostics** with support before acting. A recovery record is
not a promise that a new catalog release can be installed.

For normal app maintenance, use the app's recommended action: back up, repair,
pause, resume, restart, or uninstall. Keep using the current app release until
an explicitly qualified managed-update workflow is available.
