...
public class DiscoverSetupService {
    ...
    public DiscoverSetupSchema schema(ApplicationManifest manifest) {
        List<DiscoverSetupInput> inputs = new ArrayList<>();
        ...
        inputs.add(new DiscoverSetupInput(
                ...
                List.of(),
                Map.of()));
        return new DiscoverSetupSchema(manifest.id(), 1, inputs);
    }

    public DiscoverSetupAnswers defaults(ApplicationManifest manifest) {
        return new DiscoverSetupAnswers(schema(manifest).inputs().stream()
                .collect(java.util.stream.Collectors.toMap(
                        DiscoverSetupInput::id,
                        input -> input.defaultValue() == null ? "" : input.defaultValue(),
                        (left, right) -> right,
                        java.util.LinkedHashMap::new)));
    }

    public DiscoverSetupAnswers mergedAnswers(ApplicationManifest manifest, DiscoverSetupAnswersRequest request) {
        ...
        return new DiscoverSetupAnswers(values);
    }

    public void persist(String appId, String catalogAppId, DiscoverSetupAnswers answers) {
        setupRepository.save(appId, catalogAppId, answers);
    }

    private List<DiscoverSetupInput> appSpecificInputs(ApplicationManifest manifest) {
        return switch (manifest.id()) {
            case "grafana" -> List.of(new DiscoverSetupInput(
                    "grafanaDataSource",
                    "Dashboard source",
                    "choice",
                    "app_specific",
    ...
    private List<DiscoverSetupOption> accessOptions() {
        return List.of(
    ...
    private DiscoverSetupOption option(String value, String label, String description, boolean recommended, boolean advanced) {
        return new DiscoverSetupOption(value, label, description, recommended, advanced);
    }
}
